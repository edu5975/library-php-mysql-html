<?php
    ...
    class Editorial extends baseDatos
    {
        function Editorial(){
        ...
        function accion($cual,$id=-1){
            ...
            switch ($cual) {
                case 'delete':
                    $this->consulta("delete from editorial where id=".$id);
                    $result=$this->accion("list");//Refrescar la tabla
                    break;
                case 'list':
                    # Mostrar el listado de la tabla
                    $cad="select id, nombre from editorial";
                    //echo $cad;
                    $result=$this->showTabla($cad,true,array("edit","delete"),array("10%","80%")); //Agregar fechaultAcceso, acceso, foto, para bd amorosos
                break;
                case 'insert':
                    $this->consulta("insert into editorial set nombre='".$_POST['nombre']."'");
                    $result=$this->accion("list"); //Refrescar la tabla
                break;
                case 'update':
                    $this->consulta("UPDATE editorial set nombre='".$_POST['nombre']."' WHERE id=".$_POST['id']);
                    $result=$this->accion("list"); 
                break;
    
                case 'formEdit':
                    $row=$this->saca_tupla("SELECT * from editorial where id=".$_REQUEST['id']);    
                ...
                case 'formNew':
                    $result.='<div class="container">
                        <form method="post" >
                            ...
                            $result.=(isset($row->nombre)?'<button class="btn btn-sm- btn-success">Actualizar</button><input type="hidden" name="id" value="'.$row->id.'"/><input type="hidden" name="accion" value="editorial.update"     />':'<button class="btn btn-sm- btn-success">Registrar</button><input type="hidden" name="accion" value="editorial.insert"     />');
                    $result.= '</form>
                    </div>';
        ...
        function showTabla($query,$agregar = false,$p_iconos=array(),$p_columna=array(), $p_coloRenglon="table-dark"){
            $this->consulta($query);
            $result = '<div class="container"><span class="badge badge-primary">Editorial</span><table class="table" border = "0">';
            ...
            $result.='<td colspan = "'.count($p_iconos).'">'.(($agregar)?'<a href="?accion=editorial.new"> 
            ...
            for ($contR=0; $contR < $this->numeTuplas; $contR++) { 
                ...
                if(in_array("delete",$p_iconos)){
                    $result.= '<td><a href="?id='.$id.'&accion=editorial.delete"  width="5%"> <img style="cursor:pointer;" border="0" id="delete"/> </a></td>';
                }
                if(in_array("edit",$p_iconos)){
                    $result.= '<td><a href="?id='.$id.'&accion=editorial.formEdit"  width="5%"> <img style="cursor:pointer;" border="0" id="edit"/> </a> </td>';
                }
